...
C
      ...
      ISAM(I) = IAND(MSK1,ISHFT(NPACK(WPO),SHFT))
      GO TO 40
C
...
C
   30 CONTINUE
      BTMPL = ISHFT(IAND(NPACK(WPO),MASK(RBITS)),SHFT)
      BTMPR = IAND(ISHFT(NPACK(WPO+1),SHFT-BPERI),MASK(SHFT))
