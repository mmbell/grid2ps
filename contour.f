...
C FILL THE CONTOUR
        IF (fill_flag.EQ.'Y') Write (52,*)  '      ',count,' o fill'
...
C LOOK AT BOTTOM OF QUADRANT 2 (RIGHT SIDE OF BOX)
	  IF ((z2.LT.zct.AND.z3.GT.zct)
     X        .OR.(z2.GT.zct.AND.z3.LT.zct)) THEN
C CALCULATE END OF LINE SEGMENT
...
C LOOK AT RIGHT SIDE OF QUADRANT 2
	  IF ((z3.LT.zct.AND.z5.GT.zct)
     X         .OR.(z3.GT.zct.AND.z5.LT.zct)) THEN
...
C LOOK AT BOTTOM OF QUADRANT 3 (TOP SIDE OF BOX)
	  IF ((z3.LT.zct.AND.z4.GT.zct)
     X        .OR.(z3.GT.zct.AND.z4.LT.zct)) THEN
C CALCULATE END OF LINE SEGMENT
...
C LOOK AT RIGHT SIDE OF QUADRANT 3
	  IF ((z4.LT.zct.AND.z5.GT.zct)
     X         .OR.(z4.GT.zct.AND.z5.LT.zct)) THEN
...
C CHECK FOR BAD DATA
          IF (z1.LT.-900.OR.z2.LT.-900
     X        .OR.z3.LT.-900.OR.z4.LT.-900) THEN
           moveto = 'Y' 
	   clspath = 1
          ENDIF
	  qline_flag1=kx2
...
C LOOK AT BOTTOM OF QUADRANT 4 (LEFT SIDE OF BOX)
	  IF ((z1.LT.zct.AND.z4.GT.zct)
     X         .OR.(z1.GT.zct.AND.z4.LT.zct)) THEN
C CALCULATE END OF LINE SEGMENT
...
C LOOK AT RIGHT SIDE OF QUADRANT 4
	  IF ((z1.LT.zct.AND.z5.GT.zct)
     X        .OR.(z1.GT.zct.AND.z5.LT.zct)) THEN
...
C LOOK AT BOTTOM OF QUADRANT 1 (BOTTOM SIDE OF BOX)
	  IF ((z1.LT.zct.AND.z2.GT.zct)
     x         .OR.(z1.GT.zct.AND.z2.LT.zct)) THEN
C CALCULATE END OF LINE SEGMENT
...
C LOOK AT RIGHT SIDE OF QUADRANT 1
	  IF ((z2.LT.zct.AND.z5.GT.zct)
     X        .OR.(z2.GT.zct.AND.z5.LT.zct)) THEN
...
C BEGINNING OF LINE SEGMENT=END OF PREVIOUS LINE SEGMENT
          ...
          KY1 = KY2
C LOOK AT BOTTOM OF QUADRANT 3 (TOP SIDE OF BOX)
	  IF ((z4.LT.zct.AND.z3.GT.zct)
     X         .OR.(z4.GT.zct.AND.z3.LT.zct)) THEN
C CALCULATE END OF LINE SEGMENT
           KX2 = IX1 + NINT(XINT*((IX-1) + (ZCT-Z4)/(Z3-Z4)))
           KY2 = IY1 + NINT(YINT*IY)
C END THE LOOP IF BACK AT THE BEGINNING
...
**************************************************
	...
	Include 'common3.h'
      	...
      	Real x1,y1,qmove_flag1,qmove_flag2
	...
	IF (num.EQ.1.OR.num.GT.2) THEN
	 IF (scan_mode.EQ.'POLA') THEN
	    DO i=1,line_count,num
		  ...
		  Write (52,19) xline(i),yline(i-1),yline(i)
	       ELSE
                  Write (52,20) xline(i),yline(i),pscommand(i)
	       ENDIF
            ENDDO
	 ELSE
	    DO i=1,line_count,num
	       Write (52,21) xline(i),hkm,yline(i),vkm,pscommand(i)
	    ENDDO
         ENDIF
...
C if previous command moveto, the end with moveto else lineto
	IF (scan_mode.NE.'POLA') THEN
	   IF (pscommand(line_count-1).EQ."m") THEN
 	      Write (52,814) xline(1),hkm,yline(1),vkm,pscommand(1)
	   ELSE
	      Write (52,815) xline(1),hkm,yline(1),vkm
	   ENDIF
	ENDIF
...
C Calculate text position
	...
	End
**************************************************
	Subroutine WriteLabel (xtext_pos,ytext_pos,theta,
     X                         label_count,count)
...
C Calculate which format to use
	...
	IF ((zct_int.GT.-100.AND.zct_int.LE.-10)
     X       .OR.(zct_int.GE.100)) frmat=30
...
C can choose which label to print
	...
	DO i=1,label_count
	 ...
	 IF (frmat.EQ.10) THEN
	    IF (scan_mode.EQ.'POLA') THEN
	       Write (52,10) comment,xtext_pos(i),ytext_pos(i),
     $         theta(i),zct_int
	    ELSE
	       Write (52,11) comment,xtext_pos(i),hkm,ytext_pos(i),vkm,
     $         theta(i),zct_int
	    ENDIF
	 ELSEIF (frmat.EQ.20) THEN
	    IF (scan_mode.EQ.'POLA') THEN
	       Write (52,20) comment,xtext_pos(i),ytext_pos(i),
     $         theta(i),zct_int
	    ELSE
	       Write (52,21) comment,xtext_pos(i),hkm,ytext_pos(i),vkm,
     $         theta(i),zct_int
	    ENDIF
	 ELSEIF (frmat.EQ.30) THEN
	    IF (scan_mode.EQ.'POLA') THEN
	       Write (52,30) comment,xtext_pos(i),ytext_pos(i),
     $         theta(i),zct_int
	    ELSE
	       Write (52,31) comment,xtext_pos(i),hkm,ytext_pos(i),vkm,
     $         theta(i),zct_int
	    ENDIF
	 ELSEIF (frmat.EQ.40) THEN
	    IF (scan_mode.EQ.'POLA') THEN
	       Write (52,40) comment,xtext_pos(i),ytext_pos(i),
     $         theta(i),zct_int
	    ELSE
	       Write (52,41) comment,xtext_pos(i),hkm,ytext_pos(i),vkm,
     $         theta(i),zct_int
	    ENDIF
	 ENDIF
