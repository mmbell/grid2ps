...
c        integer*2 id(510),nid
c      PARAMETER (NID=510)
	Integer id(510),ilhd_index,panel,fieldnum,x,y
      PARAMETER (MAXPLN=262144,MAXX=512,MAXY=512)
c      DIMENSION ID(NID),ITEM(MAXPLN),RBUF(MAXX,MAXY),ILHD(10)
      DIMENSION ITEM(MAXPLN),RBUF(MAXX,MAXY),ILHD(10)
      DIMENSION SRBUF(MAXPLN)
      CHARACTER*2 FLDNAM(8)
...
C200	     IF (fieldnum.LT.5) THEN  !WHILE LOOP
	     DO fieldnum=1,4
   	     IF (fld_name(1,fieldnum).EQ.fldnam(1).AND.
     X          fld_name(2,fieldnum).EQ.fldnam(2).AND.
     X          fld_name(3,fieldnum).EQ.fldnam(3).AND.
