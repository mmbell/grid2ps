	...
	Real temp_array(512,512)

	Open (unit=54,file=data_file,status="unknown")
...
* Read array header & store in id
        Read (54,15),imax,jmax,kmax,sx,sy,sz,Olat,Olong,z0,Nmosm,Su,Sv,
...
*	  ENDDO
	...
	End
**************************************************
	Subroutine StoreDataP3(id,nid,imax,jmax,kmax,
     X    temp_array,data_format)
	...
	Integer id,nid,imax,jmax,kmax,i,j,k,m,field
...
C Read the array
	    DO m=1,5
	     IF (m.GE.1.AND.m.LT.5) THEN
	       Read (54,11),temp_array
	     ELSE
	       Read (54,12),temp_array
	     ENDIF
...
*C Read the array
	    DO m=1,6
	     Read (54,*)
             DO j=1,jmax
              read (54,11),( temp_array(i,j),i=1,imax)
             ENDDO
C Store the array in fld_data
             IF (fix_axis.EQ.'Z'.OR.fix_axis.EQ.'z') THEN
	       DO field=1,4
...
c	write(scan_mode,'(i2)')id(16),id(17)
c	print*,scan_mode,id(16),id(17)
...
C FIXED AXIS=X
	...
	End
      ...
      implicit none
      integer oldint,int1,int2,newint
      int1 = mod(oldint,256)
